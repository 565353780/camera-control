...
class MeshRenderer(object):
    ...
    def sampleRenderData(
        ...
        renderer = NVDiffRastRenderer(device)

        for i in trange(len(camera_list)):
            camera = camera_list[i]

            res = renderer.render(mesh, camera, return_types=["mask", "normal", "color", "depth"], vertices_tensor=vertices_tensor) 

            camera.image_id = f'{i:06d}.png'

            camera.loadMask(res['mask']['mask'])
            camera.loadImage(res['color']['img'])
            camera.loadNormalWorld(res['normal']['img'])
            camera.loadDepth(res['depth']['depth'])
